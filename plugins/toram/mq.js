...
let handler = async (m, { conn, usedPrefix, command, text, args }) => {
  ...
  if (!text) {
    ...
    ext += `Contoh :\n *${usedPrefix}${command} 280|1|0|111|0*\n Or\n *${usedPrefix}${command} 280/1/0/111/1* \n\n`;
  ...
  let [level, percentage, mqBegin, mqEnd, skipVenena] = text.split(/[^\w\s]/g).map(Number)
...
handler.menutoram = ["mq <text>"];
handler.tagstoram = ["toram"];
handler.command = /^(mq)$/i;
