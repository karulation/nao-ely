...
export async function before(m, { conn, text, participants }) {
    ...
    const broadcastGroups = [
        "120363022290154127@g.us",
        "60177637943-1627735681@g.us",
        "120363022632239561@g.us",
    ]; // '60177637943-1634746023@g.us' Replace with actual non-gaming group IDs
    ...
    const isOnlyEmoteOrEmpty = (text) => {
        ...
        const emojiRegex =
            /^(\p{Extended_Pictographic}|\p{Emoji}|\p{Emoji_Component}|\p{Emoji_Modifier}|\p{Emoji_Modifier_Base}|\p{Emoji_Presentation})+$/u;
    ...
    if (m.chat === groupID) {
        ...
        if (/video|image/g.test(mime) && !/webp/g.test(mime)) {
            ...
            for (const broadcastGroup of broadcastGroups) {
                const url = extractUrl(textMessage);
                if (url) {
                    await conn.sendMessage(broadcastGroup, {
                        ...
                        contextInfo: {
                            externalAdReply: {
                                ...
                                thumbnailUrl: null, // optional thumbnail
                            },
                        },
                    });
                } else {
