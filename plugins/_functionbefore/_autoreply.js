...
export async function before(m) {
	...
	if (m.text.toLowerCase() == 'bot') {
		let url = `https://raw.githubusercontent.com/clicknetcafe/Databasee/main/sfx/${sfx.getRandom()}.mp3`
		// await this.reply(m.chat, bot.getRandom())
		if (ranNumb(1, 2) > 1) await this.reply(m.chat, bot.getRandom())
		await this.sendMsg(m.chat, { audio: { url: url }, mimetype: 'audio/mpeg' }, { quoted: m })
		// await this.sendFile(m.chat,
		// 	`https://raw.githubusercontent.com/clicknetcafe/Databasee/main/sfx/${sfx.getRandom()}.mp3`,
		// 	'', '', null, true, {}, true)
	}
	if (m.text.toLowerCase() == 'ping') await this.reply(m.chat, ping.getRandom())
	if (m.text.toLowerCase() == 'p') await this.reply(m.chat, bot.getRandom())
	return !0
}
