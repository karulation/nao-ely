...
export async function before(m) {
	...
	if (m.messageStubType == 21) {
		await this.reply(
			m.chat,
			`${edtr} change group name to :\n*${m.messageStubParameters[0]}*`,
			fkontak,
			{ mentions: [m.sender] }
		);
	} else if (m.messageStubType == 22) {
		await this.reply(m.chat, `${edtr} change group icon.`, fkontak, {
			mentions: [m.sender],
		});
	} else if (
		m.messageStubType == 1 ||
		...
		await this.reply(m.chat, `${edtr} *reset* group link!`, fkontak, {
			mentions: [m.sender],
		});
	} else if (m.messageStubType == 24) {
		await this.reply(
			m.chat,
			`${edtr} change description.\n\n${m.messageStubParameters[0]}`,
			fkontak,
			{ mentions: [m.sender] }
		);
		...
		await this.reply(
			m.chat,
			`${edtr} change for *${m.messageStubParameters[0] == "on" ? "only admin" : "all members"
			}* can edit group info.`,
			fkontak,
			{ mentions: [m.sender] }
		);
		...
		await this.reply(
			m.chat,
			`${edtr} have *${ms ? "close" : "open"}* group!\nNow ${ms ? "only admin" : "all members"
			} can send message.`,
			fkontak,
			{ mentions: [m.sender] }
		);
		...
		await this.reply(
			m.chat,
			`${edtr} has kick @${m.messageStubParameters[0].split`@`[0]} from group.`,
			fkontak,
		...
		await this.reply(
			m.chat,
			`${edtr} have promote @${m.messageStubParameters[0].split`@`[0]
			} as admin.`,
			fkontak,
			{ mentions: [m.sender, m.messageStubParameters[0]] }
		);
		...
		await this.reply(
			m.chat,
			`${edtr} have demote @${m.messageStubParameters[0].split`@`[0]
			} from admin.`,
			fkontak,
			{ mentions: [m.sender, m.messageStubParameters[0]] }
		);
		...
		await this.reply(
			m.chat,
			`${edtr} change disappear messages to *@${m.messageStubParameters[0]}*`,
			fkontak,
			{ mentions: [m.sender] }
		);
		...
		await this.reply(
			m.chat,
			`${edtr} *deactivate* disappear messages.`,
			fkontak,
			{ mentions: [m.sender] }
		);
		...
		await this.reply(
			m.chat,
			`${edtr} *${ms ? "activate" : "deactivate"
			}* 'MEMBERSHIP_JOIN_APPROVAL_MODE'.`,
			fkontak,
			{ mentions: [m.sender] }
		);
		...
		let meta = await Connection.store.fetchGroupMetadata(
			id,
			this.groupMetadata
		);
		let bg = `https://raw.githubusercontent.com/clicknetcafe/Databasee/main/azamibot/media/picbot/menus/menus_${padLead(
			ranNumb(43),
			3
		)}.jpg`;
		...
		var text = (
			add
				? (
					...
					.replace("@subject", namegc)
					.replace(
						"@desc",
						chat.isBanned
							? `${chat.lastmute > 0
								? `Bot has been muted for :\n${(
									chat.mutecd -
									(new Date() - chat.lastmute)
								).toTimeString()}`
		...
		try {
			...
			pp = await new can.WelcomeLeave()
				...
				.setDescription(
					`${add ? "Hello" : "Sayonara"} ${name} | ${add ? "Welcome to" : "Leaving from"
					} ${namegc}`,
					"#34eb7d"
				)
				.setBorder("#2a2e35")
