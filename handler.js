  ...
  try {
    ...
    const isROwner =
      ...
      [
        ...
        ...db.data.datas.rowner.map(([number]) => number),
      ]
        .map((v) => v.replace(/[^0-9]/g, "") + "@s.whatsapp.net")
        .includes(m.sender);
    const isOwner =
      ...
      db.data.datas.owner
        ...
        .includes(m.sender);
    const isPrems =
      isOwner || db.data.datas.prems.map((v) => v.user).includes(m.sender);
    ...
    for (let name in plugins) {
      ...
      if (typeof plugin.all === "function") {
        try {
          ...
          if (db.data.datas.rowner.length > 0) {
            for (let [jid] of db.data.datas.rowner.filter(
      ...
      if ((usedPrefix = (match[0] || "")[0])) {
        ...
        if (m.chat in db.data.chats || m.sender in db.data.users) {
          ...
          if (m.plugin != ubc && chat?.isBanned) return; // Except this
          if (m.plugin != ubu && user?.banned) return;
        }
    ...
    if (opts["autoread"]) await this.readMessages([m.key]).catch(() => {}); // WARNING : easy to get banned
  }
}
  ...
  if (Array.isArray(message.keys) && message.keys.length > 0) {
    const tasks = await Promise.allSettled(
      message.keys.map(async (key) => {
        ...
        const msg =
          this.loadMessage(key.remoteJid, key.id) || this.loadMessage(key.id);
        ...
        await this.reply(
          ...
          `@${
            participant.split`@`[0]
          } telah menghapus pesan\n*.off antidelete* untuk menonaktifkan`,
          msg,
          { mentions: [participant] }
        );
        return await this.copyNForward(key.remoteJid, msg).catch((e) =>
          console.log(e, msg)
        );
      })
    );
...
global.dfail = (type, m, conn) => {
  let msg = {
    ...
    unreg:
      "You need to register for this feature:\n\n*#daftar nama.umur*\n\nExample: */daftar Manusia.16*",
